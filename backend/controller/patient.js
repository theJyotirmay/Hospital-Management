...
const create_appointments = async (req, res) => {
  try {
    const { doctor, chief_complaints, date, status } = req.body;

    const user = req.id;

    if (!user | !doctor | !chief_complaints | !date) {
      return res.status(202).json({ message: "incomplete content" });
    } else {
...
const create_razorpay_order = async (req, res) => {
  try {
    console.log("Creating Razorpay order for amount:", req.body.amount);
    const instance = new Razorpay({
      key_id: "rzp_test_1DP5mmOlF5G5ag", // Dummy key
      key_secret: "secret_dummy_12345",
    });
...
module.exports = { all_appointments, create_appointments, payment, user_query, ambulance_booking, single_appointments, single_user, update_user, create_razorpay_order };
